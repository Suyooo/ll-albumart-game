...
<script lang="ts">
	...
	function resetDay() {
		...
		const resetToIdx = $ALL_STATES.length - 1 - [...$ALL_STATES].reverse().findIndex((s) => s.day === DAY_TO_PLAY);
		if (
...
<div class="fixed bottom-0 right-8 w-full max-w-sm">
	<button
		class="flex w-full items-center justify-between rounded-t-xl bg-accent px-2 py-1 font-bold uppercase tracking-widest"
		on:click={() => (show = !show)}
	>
		Mod Mode
		{#if show}
	...
	{#if show}
		<div class="border-l-4 border-r-4 border-accent bg-input-background px-2 py-1" transition:slide={{}}>
			...
			{:else}
				...
				<button
					class="mt-4 flex w-full items-center justify-between rounded-t bg-accent px-2 uppercase tracking-widest"
				...
				{#if showPrevRounds}
					<div
						class="max-h-80 overflow-y-scroll rounded-b border-2 border-t-0 border-accent bg-background px-2 py-1"
						transition:slide={{}}
					>
						<table>
							{#each { length: $STATE.day - 1 } as _, i}
								...
								<tr>
									<td rowspan="2" class="pr-2 text-center align-top text-xs font-bold leading-4">
										{d}
									</td>
								...
								<tr>
									<td class="text-xs leading-3 text-subtle">
				...
				{#if showAlbumHistory}
					<div
						class="max-h-40 overflow-y-scroll rounded-b border-2 border-t-0 border-accent bg-background px-2 py-1"
						transition:slide={{}}
					>
						<table class="empty:before:content-['None']">
							{#each { length: $STATE.day - 1 } as _, i}
								...
								{#if getIdsForDay(d, true).rolledAlbumId == $STATE.albumId}
									<tr>
										<td class="pr-2 text-center align-top text-xs font-bold leading-4">
											{d}
										</td>
										<td class="text-sm leading-4">
											{GAME_POOL[getIdsForDay(d, true).rolledGameId].name}
				...
				<div class="mb-1 mt-4 flex w-full items-center justify-between">
