...
<div class="relative flex w-full items-center justify-center overflow-visible">
	<div class="mx-2 w-8 flex-shrink-0">
		<PageButton class="w-8" disabled={stage === 0} label="Previous Step" on:click={() => changeStage(-1)}>
			<Left />
		</PageButton>
	</div>
	<div class="flex max-w-sm flex-grow flex-col items-end">
		{#key maxStage}
			<div
				class="relative aspect-square w-full max-w-sm overflow-auto bg-black"
		...
		<div class="mt-2 flex w-full max-w-sm items-center">
			...
			<div class="flex-grow px-4">
				{#if $STATE.finished}
					...
					<div class="max-w-sm text-center text-xs" in:fly={{ y: -30, duration: 1000 }} aria-hidden="true">
						{ALBUM.artistEn} - <b>{@html ALBUM.titleEn}</b>
					</div>
				{:else}
					<div class="max-w-sm text-center text-xs">
