...
<div
    class="absolute left-0 top-0 w-full h-full bg-black bg-opacity-80 flex items-center justify-center px-8 pt-8 pb-24"
    on:click={closeModal}
    role="presentation"
...
>
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <div
        class="w-full max-w-md bg-background rounded p-4 relative overflow-hidden"
        on:click|stopPropagation={() => null}
        role="presentation"
    >
        <h3 class="flex-grow h-8 p-2 uppercase tracking-widest font-bold text-header-button-label">{title}</h3>
