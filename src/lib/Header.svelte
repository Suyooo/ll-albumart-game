...
<header class="mb-4 w-full border-b-2 border-header-label">
	<div class="mx-auto mt-0.5 flex max-w-screen-md items-center">
		<a class="pointer-events-none absolute left-5 opacity-0 focus:opacity-100" href="#input">Skip to game</a>
		...
		{/if}
		{#if themeShowPing}<div class="ping -ml-3 -mt-5 h-2 w-2 rounded-full bg-accent" />{/if}
		<div class="flex-grow py-2 text-center text-2xl font-bold text-header-label" aria-hidden="true">
