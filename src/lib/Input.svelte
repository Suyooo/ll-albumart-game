...
<div class="relative w-full">
	{#if showRejected}
		<div
			class="pointer-events-none absolute left-0 top-2 -mt-4 w-full -translate-y-full select-none rounded bg-guess-wrong text-center text-text"
	...
	<input
		class="w-full flex-grow rounded bg-input-background p-2 text-sm text-text ring-2 ring-inset ring-accent placeholder:text-input-placeholder focus:ring-input-border"
		id="input"
	...
	/>
	<div class="mt-2 flex w-full flex-row-reverse justify-between">
...
<style lang="postcss">
	...
	:global(.autocomplete) {
		@apply overflow-auto rounded-b border-2 border-t-0 border-input-border bg-input-dropdown;
	}

	:global(.autocomplete > div) {
		@apply w-full border-b-2 border-input-listsep p-2 leading-none text-text last:border-b-0;
	}
	...
	:global(.autocomplete mark) {
		@apply rounded bg-input-highlight text-text;
	}
	...
	:global(.autocomplete > div > div) {
		@apply ml-4 text-xs tracking-tighter text-input-placeholder;
	}
</style>
