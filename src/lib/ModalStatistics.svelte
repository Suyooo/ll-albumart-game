	...
	<div>
		{#key filter}
			{#each bars as [count, width], i}
				...
				<div class="flex h-6 items-center justify-center">
					<div
						class="flex h-full w-8 items-center justify-center border-r-2 border-text font-bold"
					...
					<div class="relative mr-3 h-full flex-grow overflow-hidden">
						<div
							class="absolute top-[10%] my-auto h-[80%] bg-subtle"
							style:width={width * 100 + "%"}
							class:!bg-accent={highlight}
							class:min-w-[2px]={count > 0}
							in:grow={{ delay: i * 25, target: width }}
						>
							<div class="absolute right-0 px-2 text-xs leading-[1.2rem]" class:left-full={width < 0.2}>
								{count}
							</div>
						</div>
					</div>
		...
		<div class="flex items-center justify-end">
			<select on:change={onFilterSelect} class:text-input-placeholder={filter === undefined}>
				...
				<option value="8,10,12,13">★ Special</option>
			</select>
		</div>
	</div>
	<div>
		<div class="flex items-center justify-between border-b-2 border-input-listsep px-2">
...
<style lang="postcss">
	select {
		@apply rounded bg-input-background px-2 py-1;
	}
</style>
