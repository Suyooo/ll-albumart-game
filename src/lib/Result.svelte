...
>
	<h2 class="text-2xl font-bold uppercase tracking-widest" class:text-accent-text={GAME.messageOverride}>
	...
	<div class="mb-3 mt-2 flex space-x-3" aria-hidden="true">
...
<div in:fly={{ x: -50, delay: 500, duration: 1000 }}>
	<PageButton on:click={shareResult} label="Share Your Result" class="h-[unset] gap-x-2 px-3 py-2">
		{#if copied}
			<Checkmark />
...
<div in:fly={{ x: -50, delay: 600, duration: 1000 }} aria-live="polite">
	{#if timerOver}
		<div class="flex items-center space-x-2 text-sm" aria-atomic="true">
