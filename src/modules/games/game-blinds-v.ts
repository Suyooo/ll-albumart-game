...
export function getGameInstance(day: number, _album: AlbumInfo, _image: Image, scaledImage: Canvas): GameInstance {
	...
	const getShareCanvas = (): Canvas => {
		...
		ctx.globalAlpha = 0.2;
		ctx.drawImage(firstGuessCanvas, positions[0] - 2, 0, 4, CANVAS_SIZE, 0, 0, 256, 256);
		ctx.globalAlpha = 1;
		ctx.drawImage(firstGuessCanvas, positions[0] - 2, 0, 4, 64, 104, 0, 16, 256);
		ctx.drawImage(firstGuessCanvas, positions[0] - 2, CANVAS_SIZE / 2 - 32, 4, 64, 120, 0, 16, 256);
		ctx.drawImage(firstGuessCanvas, positions[0] - 2, CANVAS_SIZE - 64, 4, 64, 136, 0, 16, 256);
		return canvas;
	};
