...
export interface AutocompleteSettings<T extends AutocompleteItem> {
	 ...
	 * @param {(items) => void} update - a callback function that must be called after suggestions are prepared
...
export default function autocomplete<T extends AutocompleteItem>(
	...
	container.setAttribute("aria-label", "Options");
	...
	function stringHash(s: string): number {
		return (
			s.length +
			(s.at(-1)?.charCodeAt(0) || -1) +
			(s.at(-2)?.charCodeAt(0) || -1) +
			(s.at(-3)?.charCodeAt(0) || -1)
		);
	}
	...
	function inputEventHandler(ev: KeyboardEvent): void {
		const ignore = settings.keysToIgnore || [
			"ArrowUp",
			"Enter",
			"Escape",
			"ArrowRight",
	...
	function keydownEventHandler(ev: KeyboardEvent): void {
		...
		if (pressedKey === "ArrowUp" || pressedKey === "ArrowDown" || pressedKey === "Escape") {
			...
			if (pressedKey === "Escape") {
				...
				pressedKey === "ArrowUp" ? selectPrev() : selectNext();
				update();
			}
	...
	return {
		destroy,
		open: () => startFetch(EventTrigger.Focus),
		close: clear,
	};
}
