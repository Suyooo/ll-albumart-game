...
const loadedStats = localStorage.getItem("statistics");
...
export const STATISTICS: Readable<Statistics> &
    { addNewDay: () => void, addFinishedState: (state: PlayState) => void, breakStreak: () => void } = {
    subscribe,
    addNewDay: () => update(statistics => {
        statistics.viewed++;
        return statistics;
    }),
    addFinishedState: (state) => update(statistics => {
        if (state.cleared) {
            statistics.cleared++;
